...
/**
 * /src/Api/V1/Triggers/Atividade/Trigger0011.php.
 *
...
class Trigger0011 implements TriggerInterface
{
    ...
    {
        if ('devolver_submissao' === $restDto->getDestinacaoMinutas()) {
            $this->transactionManager->addContext(
                new Context(
                    'atividadeDesaprovacao',
                    [
            ...
            try {
                $nomeEspecieAtividade = $this->parameterBag->get(
                    'supp_core.administrativo_backend.especie_atividade_devolver_submissao_'.mb_strtolower(
                        $restDto->getTarefa()->getEspecieTarefa()->getGeneroTarefa()->getNome()
                    )
                );
            } catch (Throwable) {
                $nomeEspecieAtividade = $this->parameterBag->get(
                    'supp_core.administrativo_backend.especie_atividade_devolver_submissao_administrativo'
                );
            }
            ...
            $tarefaDTO->setSetorResponsavel($restDto->getTarefa()->getSetorOrigem());
            $tarefaDTO->setUsuarioResponsavel($restDto->getTarefa()->getCriadoPor());
            ...
            $tarefaDesaprovacao = $this->tarefaResource->create($tarefaDTO, $transactionId);
            ...
            foreach ($restDto->getTarefa()->getMinutas() as $documento) {
                ...
                if (!$documento->getJuntadaAtual()
                    ...
                    $documentoDTO->setTarefaOrigem($tarefaDesaprovacao);
                    ...
                    $vinculacaoEtiquetaDTO->setTarefa($tarefaDesaprovacao);
            ...
            $compartilhamentoDTO->setTarefa($tarefaDesaprovacao);
            ...
            $this->criaCompartilhamentoFluxo($restDto->getTarefa(), $tarefaDesaprovacao, $transactionId);

            $this->transactionManager->removeContext('atividadeDesaprovacao', $transactionId);
        }
    }
