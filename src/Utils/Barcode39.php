 ...
 *
 * @see http://www.shayanderson.com/php/php-barcode-generator-class-code-39.htm
 *
...
final class Barcode39
{
    ...
    public function draw($filename = null)
    {
            ...
            if (isset($this->_codes_39[ord($v)])) {
                ...
                if ($code) {
                    ...
                    for ($j = 0; $j < strlen($code); ++$j) {
                        ...
                        if (2 == strlen($f2)) {
                            // set bar fill
                            $fill = self::f2B == $f2 || self::f2b == $f2 ? '_000' : '_fff';
                            // set bar width
                            $w = self::f2B == $f2 || self::f2W == $f2 ? $this->barcode_bar_thick : $this->barcode_bar_thin;
                            // check for valid bar params
                            if ($w && $fill) {
                                // add bar coordinates and params
                                $bars[] = [$pos, $this->barcode_padding, $pos - 1 + $w,
        ...
        if ($this->barcode_text) {
            ...
            // draw barcod text
            //var_dump($pos_center);die();
            imagestring($img, $font_size, intval($pos_center), $this->barcode_height - $barcode_text_h - 2,
                $barcode_string, $_000);
        }
        ...
        // display image
            ...
            ob_end_clean();
            echo '<img src="data:image/png;base64,'.base64_encode($imagedata).'" alt="image 1" />';
        }
