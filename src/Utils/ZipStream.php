 ...
 *
 * Inspired by CreateZipFile by Rochak Chauhan www.rochakchauhan.com
 ...
 *
 * @see http://www.phpclasses.org/package/6110
...
class ZipStream
{
    ...
    private int $zipMemoryThreshold = 1_048_576; // Autocreate tempfile if the zip data exceeds 1048576 bytes (1 MB)
    private ?string $zipData = null;
    private $zipFile = null;
    private ?string $zipComment = null;
    private array $cdRec = []; // central directory
    ...
    private ?string $streamFileComment;
    private ?int $streamTimestamp;
    ...
    /**
     * Extra fields on the Zip directory records are Unix time
     * codes needed for compatibility on the default Mac zip archive tool.
    ...
    {
        $this->addExtraField = (true === $setExtraField);
    }
     ...
     * @param string $fileComment   (Optional) Comment to be added to the archive for
     *                              this directory. To use fileComment, timestamp must be given.
     *
     * @return bool $success
     */
    ...
    public function addDirectoryContent($realPath, $zipPath, $recursive = true, $followSymlinks = true, &$addedFiles = [])
    {
        if (file_exists($realPath) && !isset($addedFiles[realpath($realPath)])) {
            ...
            foreach ($iter as $file) {
                ...
                if (file_exists($newRealPath) && (true === $followSymlinks || !is_link($newRealPath))) {
                    if ($file->isFile()) {
    ...
    public function addLargeFile($dataFile, $filePath, $timestamp = 0, $fileComment = null)
    {
        ...
        } elseif (is_resource($dataFile) && 'stream' == get_resource_type($dataFile)) {
            $fh = $dataFile;
     ...
     * @param string $data
     *
     * @return bool|false|int $length bytes added or false if the archive is finalized or there are no open stream
     */
    public function addStreamData($data)
    {
        if ($this->isFinalized || 0 == strlen($this->streamFilePath)) {
            return false;
        }
    ...
    {
        if (!$this->isFinalized) {
            if ($this->streamFilePath && strlen($this->streamFilePath) > 0) {
                $this->closeStream();
            }
     ...
     *
     * @return int|mixed $size Size of the archive
     */
     ...
     * @param int $timestamp
     *
     * @return false|string 2-byte encoded DOS Date
     */
    private function getDosTime($timestamp = 0)
    {
        ...
        $date = (0 == $timestamp ? getdate() : getdate($timestamp));
        date_default_timezone_set($oldTZ);
        if ($date['year'] >= 1_980) {
            return pack('V', (($date['mday'] + ($date['mon'] << 5) + (($date['year'] - 1_980) << 9)) << 16) |
    ...
    private function buildZipEntry($filePath, $fileComment, $gpFlags, $gzType, $timestamp, $fileCRC32, $gzLength, $dataLength, $extFileAttr)
    {
        ...
        $timestamp = (int) $timestamp;
        $timestamp = (0 == $timestamp ? time() : $timestamp);
        ...
        if (!isset($gpFlags) || 2 != strlen($gpFlags)) {
            $gpFlags = "\x00\x00";
        }
        ...
        $cdEntry .= (0 === $dataLength ? "\x0A\x00" : self::ATTR_VERSION_TO_EXTRACT);
        $cdEntry .= $header;
