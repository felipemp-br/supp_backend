...
class RepositoryHelper
{
     ...
     *
     * @see \SuppCore\AdministrativoBackend\Rest\Repository::getExpression
     * @see \SuppCore\AdministrativoBackend\Api\V1\Controller\Rest::processCriteria
     *
     ...
     *
     * @see \SuppCore\AdministrativoBackend\Api\V1\Controller\Rest::getSearchTerms
     *
     ...
     *      'and' => [
     *          ['u.nome', 'eq',   'foo bar']
     *          ['u.assinaturaHTML',   'neq',  'not this one']
     *      ]
     *  ]                                                   (u.nome = ?1 AND u.assinaturaHTML <> ?2)
     *  [
     *      'or' => [
     *          ['u.nome', 'eq',   'foo bar']
     *          ['u.assinaturaHTML',   'neq',  'not this one']
     *      ]
     *  ]                                                   (u.nome = ?1 OR u.assinaturaHTML <> ?2)
     *
    ...
    private static function processExpression(QueryBuilder $queryBuilder, Composite $expression, array $criteria): void
    {
        $iterator = static function ($comparison, $key) use ($queryBuilder, $expression): void {
            $expressionAnd = ('and' === $key || array_key_exists('and', $comparison));
            $expressionOr = ('or' === $key || array_key_exists('or', $comparison));
    ...
    private static function determineComparisonAndParameters(QueryBuilder $queryBuilder, array $comparison): array
    {
        ...
        if (!('isnull' === $lowercaseOperator || 'isnotnull' === $lowercaseOperator)) {
    ...
    private static function getParameters(
        ...
        if ('between' === $lowercaseOperator) {
            ...
            $parameters[] = array_map(fn ($value): Literal => $queryBuilder->expr()->literal($value), $value);
        }
    ...
    private static function getIterator(array &$condition): Closure
    {
        return function ($value, $column) use (&$condition): void {
            ...
            if (0 === strcmp($column, 'and') || 0 === strcmp($column, 'or')) {
