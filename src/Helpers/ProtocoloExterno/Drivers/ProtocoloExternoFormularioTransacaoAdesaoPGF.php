...
/**
 * src/Helpers/ProtocoloExterno/Drivers/ProtocoloExternoFormularioTransacaoAdesaoPGF.php
 *
...
/**
 * Class ProtocoloExternoFormularioTransacaoAdesaoPGF.
 *
 * @author Advocacia-Geral da União <supp@agu.gov.br>
 */
class ProtocoloExternoFormularioTransacaoAdesaoPGF extends ProtocoloExternoBasico
{
    /**
    ...
    public function validate(array $dadosRequerimento): bool
    {
        ...
        if ($errors->count()) {
            throw new Exception("CPF/CNPJ do aderente inválido!");
        }

        if ($dadosRequerimento['email'] !== $dadosRequerimento['confirmeEmail']) {
            throw new Exception("E-mail's informados do aderente não conferem!");
        }

        if ($dadosRequerimento['cpfSolicitante']) {
            $errors = $this->validator->validate(
                preg_replace('/\D/', '', $dadosRequerimento['cpfSolicitante']),
                [new CpfCnpj()]
            );

            if ($errors->count()) {
                throw new Exception("CPF do solicitante inválido!");
            }
        }

        if ($dadosRequerimento['emailSolicitante'] !== $dadosRequerimento['confirmeEmailSolicitante']) {
            throw new Exception("E-mail's informados do solicitante não conferem!");
        }
        ...
        if ($errors->count()) {
            throw new Exception("Representante ".$errors->get(0)->getMessage());
        }
    ...
    public function getDadosProtocoloExterno(
        ...
        if (!$this->suppParameterBag->has('supp_core.administrativo_backend.formularios.transacao_adesao_pgf')) {
        ...
        $dadosProtocoloExternoPGF = $this->suppParameterBag->get(
            'supp_core.administrativo_backend.formularios.transacao_adesao_pgf'
        );

        return match ($formulario->getSigla()) {
            'requerimento_transacao_adesao_pgf' => $this->getDadosProtocoloExternoFormularioTransacao(
                $dadosProtocoloExterno,
                $dadosFormulario,
                $dadosProtocoloExternoPGF
            ),
            'requerimento_transacao_informacoes_pgf' => $this->getDadosProtocoloExternoFormularioTransacaoInformacoes(
                $dadosProtocoloExterno,
                $dadosFormulario,
                $dadosProtocoloExternoPGF
            ),
     ...
     * @throws Exception
     */
    private function getDadosProtocoloExternoFormularioTransacao(
        ...
        $configSetorUnidadePorUFUsuario = current(
            array_filter(
                $dadosProtocoloExternoPGF['distribuicao_transacao'],
        ...
        $credor = current(
            array_filter(
                ...
                fn($c) => $dadosFormulario['credor'] === $c['nome']
            )
        );
     ...
     * @throws Exception
     */
    private function getDadosProtocoloExternoFormularioTransacaoInformacoes(
        DadosProtocoloExterno $dadosProtocoloExterno,
        $dadosFormulario,
        $dadosProtocoloExternoPGF
    ): ?DadosProtocoloExterno {
        ...
        $configSetorUnidadePorUFUsuario = current(
            array_filter(
                $dadosProtocoloExternoPGF['distribuicao_transacao_informacoes'],
                fn($c) => in_array($estadoEntity->getUf(), $c['uf_domicilio_devedor'])
            )
        );
    ...
    public function supports(FormularioEntity $formulario): bool
    {
        return in_array(
            $formulario->getSigla(),
            ['requerimento_transacao_adesao_pgf', 'requerimento_transacao_informacoes_pgf']
        );
    }
