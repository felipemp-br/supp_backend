...
use DOMXPath;
use ErrorException;
use SuppCore\AdministrativoBackend\BeSimple\SoapCommon\Cache;
...
class WsdlDownloader
{
     ...
     */
    protected bool $resolveRemoteIncludes = true;
     ...
     * @param int  $cacheWsdl             Cache constant
     */
    public function __construct(Curl $curl, $resolveRemoteIncludes = true, $cacheWsdl = Cache::TYPE_DISK)
    {
        ...
        $this->cacheEnabled = Cache::TYPE_NONE === $cacheWsdl ? Cache::DISABLED : Cache::ENABLED == Cache::isEnabled();
    ...
    public function download($wsdl)
    {
        ...
        if ($isRemoteFile || $this->resolveRemoteIncludes) {
            ...
            if (!$this->cacheEnabled || !file_exists($cacheFilePath) || (filemtime(
                $cacheFilePath
            ) + $this->cacheTtl) < time()) {
                if ($isRemoteFile) {
                    // execute request
                    ...
                    if ($responseSuccessfull) {
                        ...
                        if ((false === strpos($response, 'wsdl:definitions')) && (false === strpos(
                            $response,
                            'schema'
                        ))) {
                            throw new ErrorException("SOAP-ERROR 1: Parsing WSDL: Couldn't load from '".$wsdl."'");
                        }
                        ...
                        throw new ErrorException("SOAP-ERROR 2: Parsing WSDL: Couldn't load from '".$wsdl."'");
                    }
                } elseif (file_exists($wsdl)) {
                    $response = file_get_contents($wsdl);
                    $this->resolveRemoteIncludes($response, $cacheFilePath);
                } else {
                    throw new ErrorException("SOAP-ERROR 3 : Parsing WSDL: Couldn't load from '".$wsdl."'");
                }
            }
        ...
        throw new ErrorException("SOAP-ERROR 4: Parsing WSDL: Couldn't load from '".$wsdl."'");
    }
