...
class Curl
{
     ...
     * @var string
     */
    private const USER_AGENT = 'PHP-SOAP/\BeSimple\SoapClient';

    public const USER_AGENT_DEV = 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/113.0';
    /**
     * Curl resource.
     *
     * @var resource
     ...
     */
    private int $followLocationMaxRedirects;

    /**
     * Request response data.
     */
    private null|bool|string $response;
     ...
     */
    public function __construct(array $options = [], int $followLocationMaxRedirects = 10)
    {
        // set the default HTTP user agent
        if (!isset($options['user_agent'])) {
            $options['user_agent'] = 'dev' === getenv('APP_ENV') ? self::USER_AGENT_DEV : self::USER_AGENT;
        }
        ...
        if (isset($options['proxy_host'])) {
            if (false !== $options['proxy_host']) {
                $proxyHost = $options['proxy_host'].($options['proxy_port'] ?? 8_080);
            } else {
                $proxyHost = false;
            }
        ...
        if (isset($options['login'])) {
            curl_setopt(
                $this->ch,
                CURLOPT_HTTPAUTH,
                $options['extra_options']['http_auth'] ?? CURLAUTH_ANY
            );
     ...
     * @param string|null $request        Request body
     ...
     */
    private function execManualRedirect(int $redirects = 0): bool|int|string|array
    {
     ...
     * @return array
     */
    protected function getErrorCodeMapping(): array
    {
        return [
            1 => 'Unknown protocol. Only http and https are allowed.',
            // CURLE_UNSUPPORTED_PROTOCOL
            3 => 'Unable to parse URL',
            // CURLE_URL_MALFORMAT
            5 => 'Could not connect to host',
            ...
            28 => 'Failed Sending HTTP SOAP request',
            // CURLE_OPERATION_TIMEDOUT
            35 => 'Could not connect to host',
            // CURLE_SSL_CONNECT_ERROR
            41 => 'Can\'t uncompress compressed response',
            // CURLE_FUNCTION_NOT_FOUND
            51 => 'Could not connect to host',
            // CURLE_PEER_FAILED_VERIFICATION
            52 => 'Error Fetching http body, No Content-Length, connection closed or chunked data',
            // CURLE_GOT_NOTHING
            53 => 'SSL support is not available in this build',
            // CURLE_SSL_ENGINE_NOTFOUND
            54 => 'SSL support is not available in this build',
            // CURLE_SSL_ENGINE_SETFAILED
            55 => 'Failed Sending HTTP SOAP request',
            // CURLE_SEND_ERROR
            56 => 'Error Fetching http body, No Content-Length, connection closed or chunked data',
            // CURLE_RECV_ERROR
            58 => 'Could not connect to host',
            // CURLE_SSL_CERTPROBLEM
            59 => 'Could not connect to host',
            // CURLE_SSL_CIPHER
            60 => 'Could not connect to host',
            // CURLE_SSL_CACERT
            61 => 'Unknown Content-Encoding',
            // CURLE_BAD_CONTENT_ENCODING
            65 => 'Failed Sending HTTP SOAP request',
            // CURLE_SEND_FAIL_REWIND
            66 => 'SSL support is not available in this build',
            // CURLE_SSL_ENGINE_INITFAILED
            67 => 'Could not connect to host',
            // CURLE_LOGIN_DENIED
            77 => 'Could not connect to host',
